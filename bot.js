...
client.on('message', msg => {
	...
	if ((mesc.startsWith("Initiating vote for ")) && (dbvars[2] === 1) && (msg.author.id === '607520778178527246')) {
		...
		collector.on('collect', rct => {//${rct.emoji.name}
			...
			if (rct.emoji.name === '✅') {yee++; console.log(`${DELTAS.members.get(lru).tag} voted yee!`); console.log(rct.users.map(u => u.id))}
			if (rct.emoji.name === '🚫') {boo++; console.log(`${DELTAS.members.get(lru).tag} voted boo!`); console.log(rct.users.map(u => u.id))}
			vtd.push(lru)
